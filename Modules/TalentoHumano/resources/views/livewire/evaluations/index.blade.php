<div class="row">
    <div class="col-sm-12">
        <x-card-table :tittle="'Información de Evaluaciones'" :exportable="true" :audit="$audit">
            <x-slot name="button">                
                ...
                @can('evaluation create')
                    <button class="btn btn-icon btn-sm btn-primary" wire:click="$set('showModalEvaluation', true)">
            ...
            <x-table.table :audit="$audit" wire:model="showauditor">
                <x-slot name="head">                    
                    ...
                    <x-table.th class="text-center" field="start_date">Fecha Inicio</x-table.th>
                    <x-table.th class="text-center" field="end_date">Fecha Final</x-table.th>
                    <x-table.th class="text-center" field="date">Fecha Evaluación</x-table.th>
                    <x-table.th class="text-center" field="result">Resultado</x-table.th>
                    <x-table.th field="interpretation">Interpretación</x-table.th>
                </x-slot>
                @forelse ($evaluations as $key => $item)
                    <tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}">
                        ...
                        <td class="p-1 text-center">{{ $item->date->format('d/m/Y') }}</td>
                        <td class="p-1 text-center">{{ $item->result ? Number::format($item->result, precision: 1) : '' }}</td>
                        <td class="p-1">{{ $item->interpretation }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7">
    ...
    @include('talentohumano::livewire.evaluations.form')
</div>
@push('scripts')
